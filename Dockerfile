...
# Copy verified pixi binary
COPY --from=pixi-installer /usr/local/bin/pixi /usr/local/bin/pixi
COPY scripts /opt/scripts

# Create nebari user (UID 1000, GID 100)
# Note: Ubuntu 24.04 has default 'ubuntu' user (UID 1000), we'll remove it and create nebari
# Note: users group (GID 100) already exists in Ubuntu 24.04
